...
- Ограничение Heroku: максимальное время ответа на запрос - 30 секунд. Из-за этого невозможно дождаться результата от GPT-Image-1(занимает примерно 60-90 секунд) и вернуть его(то есть на деплое не работает edit image with ai). По этому на видео я делаю на локалхосте.
